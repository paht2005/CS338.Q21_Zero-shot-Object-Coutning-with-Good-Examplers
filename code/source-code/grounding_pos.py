...
import inflect  # used for noun singularization
...
if os.path.exists(model_weights_path):
    ...
    print("WARNING: Classifier weights not found, the model will run with reduced accuracy.")
...
def is_valid_patch(patch, binary_classifier, preprocess_b32, device):
    ...
    return prob.item() > 0.8  # Single-object filter threshold

# --- READ PROMPT TXT FILE ---
def load_image_specific_prompts(txt_path):
    prompts_dict = {}
    if not os.path.exists(txt_path):
        print(f"WARNING: Prompt file not found at {txt_path}")
        return prompts_dict
...
def process_images(class_file_path, dataset_path, model, preprocess_b32, preprocess_l14, clip_model_l14, image_prompts_dict, device='gpu'):
    ...
    with open(class_file_path, 'r') as f:
        for line in f:
            ...
            # Singularize the class name (e.g. "keys" -> "key", "cars" -> "car")
            singular_name = p.singular_noun(class_name)
            if not singular_name:  # `False` means the word is already singular
            ...
            # === STEP 1: Run Grounding DINO with the base class name ===
            ...
            # Filter and score boxes obtained from the base prompt
            ...
            for i, (box, gd_logit) in enumerate(zip(patches, logits)):
                ...
                # Score is the GroundingDINO logit for the base prompt
                candidate_patches_base.append((box_np, gd_logit.item()))
            
            # Sort by logits and keep top 5
            ...
            # === STEP 2: If fewer than 5 boxes, fall back to Rich Prompt + CLIP ===
            if len(top_boxes) < 5:
                print(f"  -> Step 2: Not enough boxes, using rich prompt + CLIP")
                
                # Pull the rich prompt for this image, if any
                ...
                # Re-run prediction with the rich prompt
                try:
                ...
                # Pre-compute the CLIP L/14 text feature used as the scoring anchor
                ...
                # Track existing boxes to avoid duplicates
                ...
                for i, (box, gd_logit) in enumerate(zip(patches_rich, logits_rich)):
                    ...
                    # Skip duplicates already added by Step 1
                ...
                # Sort and pick enough rich-prompt boxes to reach top-5
            ...
            # Convert to the final list format
            final_boxes = [box for box, score in top_boxes]
            
            # Pad with dummy boxes if still fewer than 5
            while len(final_boxes) < 5:
                if len(final_boxes) > 0:
                    final_boxes.append(final_boxes[-1])
                else:
                    # Tiny dummy box at the top-left corner
...
def main(args):
    ...
    # Original input file (image -> class mapping)
